...
CONTAINS
  ...
  SUBROUTINE seq_infodata_Init( infodata, nmlfile, ID, pioid, cpl_tag)
    ...
    namelist /seq_infodata_inparm/  &
         ...
         ocn_surface_flux_scheme, alb_cosz_avg, & !+tht cosz
    ...
    if (seq_comm_iamroot(ID)) then
       ...
       infodata%alb_cosz_avg          = alb_cosz_avg !+tht
  ...
  SUBROUTINE seq_infodata_GetData_explicit( infodata, cime_model, case_name, case_desc, timing_dir,  &
    ...
    logical,                optional, intent(OUT) :: alb_cosz_avg            !+tht
    ...
    if ( present(alb_cosz_avg)   ) alb_cosz_avg   = infodata%alb_cosz_avg !+tht
  ...
  SUBROUTINE seq_infodata_PutData_explicit( infodata, cime_model, case_name, case_desc, timing_dir,  &
       ...
       do_budgets, do_histinit, drv_threading, flux_diurnal,              &
       ocn_surface_flux_scheme, alb_cosz_avg, & !+tht
       coldair_outbreak_mod,                                                           &
       flux_convergence, flux_max_iteration,                              &
    ...
    logical,                optional, intent(IN)    :: alb_cosz_avg            !+tht
    ...
    if ( present(alb_cosz_avg)   ) infodata%alb_cosz_avg   = alb_cosz_avg !+tht
  ...
  subroutine seq_infodata_bcast(infodata,mpicom)
    ...
    call shr_mpi_bcast(infodata%alb_cosz_avg,            mpicom) !+tht
  ...
  SUBROUTINE seq_infodata_print( infodata )
    ...
    write(logunit,F0L) subname,'alb_cosz_avg             = ', infodata%alb_cosz_avg !+tht
